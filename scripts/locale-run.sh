#!/bin/bash
function local_tmux {
  tmux new -d -s "$1" "node $2 --users $3 --contacts $4 --login_interval $6 --action_interval 30 >$5 2>&1"
}
...
case $1 in
 'friend') 
     for x in {0..2}; do
       c_i=$(( ( $x + 1 ) % 3  ))
       local_tmux friend_$x /opt/msgflood/js/msg.js /tmp/u-$x.txt /tmp/c-$c_i.txt /tmp/f"$x"_test.log 0.6
     done
     ;;
